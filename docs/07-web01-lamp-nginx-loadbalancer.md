...
```bash
sudo sed -n '/^http {/,/^}/p' /opt/nginx/conf/nginx.conf | grep -n "server {"
```
Open the file and delete (or comment out with `#`) the entire default `server { ... }` block found inside the `http {}` context — everything from its opening `server {` to its matching closing `}` — leaving only the `include sites-enabled/*.conf;` line to define server blocks:
...
```
> <img width="681" height="265" alt="image" src="https://github.com/user-attachments/assets/2f900c28-2da9-4018-bfc1-83ee585e8338" />
