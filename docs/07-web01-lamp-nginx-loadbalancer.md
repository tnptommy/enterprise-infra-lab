...
The golden baseline's OS disk was 20 GB; it was expanded to 50 GB in Step 1. Grow the LVM volume and filesystem to use the new space, per [`05`'s expansion procedure](./05-golden-baseline-rocky-linux-10.md#cloning-this-baseline-later) (confirm the correct partition number with `lsblk` first — 3 for UEFI layouts, 2 for BIOS):
...
df -h /
```
Expect roughly 30 GB total — the remaining 20 GB of Disk 1 is reserved for swap, created next.
...
```bash
lsblk
# Confirm the new disk shows up, e.g. as /dev/sdb, with no existing partitions
...
echo 'include sites-enabled/*.conf;' | sudo tee -a /opt/nginx/conf/nginx.conf
```

> `load_module` must be loaded once at the top level of `nginx.conf`, not inside a `server` block — move that line to the very top of `/opt/nginx/conf/nginx.conf` if `nginx -t` reports a directive-placement error.
