...
outputs:
  - eve-log:
      ...
      filename: eve.json
```
This is the structured log format that Filebeat will later ship to LOG01 and/or Wazuh — see [Forward pointers](#forward-pointers-filebeat-and-wazuh-integration) below.
...
- [`17-agent-deployment-all-vms.md`](./17-agent-deployment-all-vms.md) installs Filebeat on WEB01 with its built-in Suricata module enabled, shipping `eve.json` to LOG01 for centralized search and dashboards in Kibana.
